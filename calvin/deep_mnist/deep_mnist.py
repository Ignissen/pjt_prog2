...
with tf.Session() as sess:
  ...
  image_to_predict = Image.open("4_handdrawn.png").convert("L")
  image_to_predict = np.resize(image_to_predict, (28,28,1))
  image_to_predict = np.array(image_to_predict).astype("float32")
  image_to_predict = image_to_predict.reshape(1, 784)
  correct_prediction = tf.argmax(y,1)
  best_guess = sess.run(correct_prediction, feed_dict={x: image_to_predict, y_: mnist.test.labels, keep_prob: 1.0})
  
  print("Best guess is: ", best_guess[0])
